    ...
    implements IMavenFlexPlugin
{
    ...
    {
        IPath[] resourceLocations = facade.getResourceLocations();
        final List<IPath> classPath = new ArrayList<IPath>();

        for ( IPath iPath : resourceLocations )
        {
    ...
    /* (non-Javadoc)
     * @see net.flexmojos.m2e.maven.IMavenFlexPlugin#getFullPath()
     */
    @Override
    public IPath getFullPath( File file )
    {
        return facade.getFullPath( file );
    }
}
